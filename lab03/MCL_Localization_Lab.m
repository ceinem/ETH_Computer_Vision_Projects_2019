...
measurement = door1 + door2 + door3;
measurement = measurement/sum(measurement);
...
if(simpar.circularHallway == 1)
    ...
    predict_particles = priorbelief_particles + motion + motion_disturbance;
end
...
door1 = pdf('Normal', predict_particles, simpar.door_locations(1), simpar.door_stdev);
door2 = pdf('Normal', predict_particles, simpar.door_locations(2), simpar.door_stdev);
door3 = pdf('Normal', predict_particles, simpar.door_locations(3), simpar.door_stdev);

particle_weights = door1 + door2 + door3;
if(zk==0)
...
% TODO: Change this value for the correct one
updated_belief_particles = zeros(size(predict_particles));
random_numbers = rand(1,simpar.numberOfParticles);
Beta_cum = cumsum(particle_weights);
for i = 1:simpar.numberOfParticles
    updated_belief_particles(i) = predict_particles(find(random_numbers(i) <= Beta_cum,1));
end
