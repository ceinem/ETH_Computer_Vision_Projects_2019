...
% interest.
bbox = [0.3 -0.3 -2; 2.3 1.3 3.0]; % [minX minY minZ; maxX maxY maxZ];
volumeX = 128;
volumeY = 128;
volumeZ = 256;
volumeThreshold = 17;
...
if computeVisualHull
    ...
    for n = 1:numCameras
        %Go through all points within the colume
        for i = 1:volumeX
            for j = 1:volumeY
                for k = 1:volumeZ
                    %Compute projection into image
                    image_cor = Ps{n} * T * [i; j; k; 1];
                    %Normalize homogenous coordinates
                    image_cor = image_cor / image_cor(end);
                    
                    %Check if projection is within image plane
                    image_cor_x = round(image_cor(2));
                    image_cor_y = round(image_cor(1));
                    if image_cor_x > 0 && image_cor_x < size(sils{n},2) && image_cor_y > 0 && image_cor_y < size(sils{n},1)
                        if sils{n}(image_cor_x,image_cor_y) == 1
